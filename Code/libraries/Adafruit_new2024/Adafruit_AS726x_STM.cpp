...
#include "Adafruit_AS726x_STM.h"

Adafruit_AS726x_STM::~Adafruit_AS726x_STM(void) {
  if (i2c_dev)
    delete i2c_dev;
}
...
*/
/**************************************************************************/
bool Adafruit_AS726x_STM::begin(TwoWire *theWire) {
    Serial.println("stmtest");
    if (i2c_dev){
        delete i2c_dev;
        Serial.println("deleted i2c_dev");
    }
    
  i2c_dev = new Adafruit_I2CDevice(_i2caddr, theWire);
    Serial.println("created i2c_dev");
  if (!i2c_dev->begin()) {
    return false;
      Serial.println(" i2c_dev init failed");
  }
    
    
  _control_setup.RST = 1;
  virtualWrite(AS726X_CONTROL_SETUP, _control_setup.get());
    Serial.println("virt write to sensor config passed");
  ...
  uint8_t version = virtualRead(AS726X_HW_VERSION);
    Serial.println("virt read to version support passed");
  ...
  setDrvCurrent(LIMIT_12MA5);
    Serial.println("sets drv current");
  drvOff();
    Serial.println("drv off");

  setIntegrationTime(50);
    Serial.println("sets integration time");

  setGain(GAIN_64X);
    Serial.println("sets gain");

  setConversionType(ONE_SHOT);
    Serial.println("sets conversion type");
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::drvOn() {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::drvOff() {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::setDrvCurrent(uint8_t current) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::indicateLED(boolean on) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::setIndicateCurrent(uint8_t current) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::setConversionType(uint8_t type) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::setGain(uint8_t gain) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::setIntegrationTime(uint8_t time) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::enableInterrupt() {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::disableInterrupt() {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::startMeasurement() {
...
*/
/**************************************************************************/
uint16_t Adafruit_AS726x_STM::readChannel(uint8_t channel) {
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::readRawValues(uint16_t *buf, uint8_t num) {
  for (int i = 0; i < num; i++) {
    switch (i) {
    case AS726x_VIOLET:
      buf[i] = readViolet();
...
*/
/**************************************************************************/
void Adafruit_AS726x_STM::readCalibratedValues(float *buf, uint8_t num) {
  for (int i = 0; i < num; i++) {
    switch (i) {
    case AS726x_VIOLET:
...
*/
/**************************************************************************/
float Adafruit_AS726x_STM::readCalibratedValue(uint8_t channel) {
  uint32_t val = 0;
...
void Adafruit_AS726x_STM::write8(byte reg, byte value) {
  this->write(reg, &value, 1);
}

uint8_t Adafruit_AS726x_STM::read8(byte reg) {
  uint8_t ret;
  this->read(reg, &ret, 1);
...
void Adafruit_AS726x_STM::virtualWrite(uint8_t addr, uint8_t value) {
...
void Adafruit_AS726x_STM::read(uint8_t reg, uint8_t *buf, uint8_t num) {
  uint8_t buffer[1] = {reg};
  i2c_dev->write_then_read(buffer, 1, buf, num);
}

void Adafruit_AS726x_STM::write(uint8_t reg, uint8_t *buf, uint8_t num) {
  uint8_t buffer[1] = {reg};
